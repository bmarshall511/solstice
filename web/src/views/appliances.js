...
function drawDial(S) {
  ...
  $('schDeltas').innerHTML = onPlan ? `<div style="grid-column:1/-1"><small>You're on the recommended schedule</small><b>${P.hours} h at ${sp.filterRpm.toLocaleString()} RPM${P.boostHours ? ` + ${P.boostHours} h boost` : ''}</b><span>applied ${appliedAt} · ${P.kwhPerDay} kWh a day</span></div>`
    : `<div><small>Electricity</small><b>${dk >= 0 ? '−' : '+'}${Math.abs(dk).toFixed(dk % 1 ? 1 : 0)} kWh</b><span>per day</span></div><div><small>Cost</small><b>${dc == null ? '—' : `${dc >= 0 ? '−' : '+'}$${Math.abs(dc)}`}</b><span>per month</span></div>
  ...
  $('schActions').innerHTML = onPlan ? `<p class="fine" style="margin-top:10px"><button class="link" id="poolRestore" style="margin:0;padding:4px 10px">Restore the previous schedule</button></p>`
