...
function drawPlan(S, sim, outdoor, sunH) {
  ...
  $('acDeltas').innerHTML = `<div><small>AC electricity</small><b>${P.kwhSaved ? '−' + P.kwhSaved + ' kWh' : '0 kWh'}</b><span>per day</span></div><div><small>Cost</small><b>${S.guest ? veil() : P.costSavedMonth == null ? '—' : P.costSavedMonth ? '−' + money(P.costSavedMonth) : '$0'}</b><span>per month</span></div>
