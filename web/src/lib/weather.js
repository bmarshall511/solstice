import { siteLocation } from './util.js';

/** The site's coordinates arrive with /api/settings at boot; until then every call below rejects instead of guessing. */
const at = () => { const s = siteLocation(); if (!s) throw new Error('Site location not loaded yet'); return s; };
const base = () => { const s = at(); return `latitude=${s.lat}&longitude=${s.lon}&timezone=America%2FChicago`; };
...
export const forecast = async () => fetch(`https://api.open-meteo.com/v1/forecast?${base()}&${tilt}&past_days=31&forecast_days=3&temperature_unit=fahrenheit&wind_speed_unit=mph` +
...
export const archive = async (from, to) => fetch(`https://archive-api.open-meteo.com/v1/archive?${base()}&${tilt}&start_date=${from}&end_date=${to}&temperature_unit=fahrenheit` +
  `&hourly=global_tilted_irradiance&daily=temperature_2m_max,temperature_2m_mean,precipitation_sum`).then(r => r.json());

export const nwsAlerts = async () => { const s = at(); return fetch(`https://api.weather.gov/alerts/active?point=${s.lat},${s.lon}`).then(r => r.json()).then(j => j.features?.map(f => f.properties) ?? []); };
