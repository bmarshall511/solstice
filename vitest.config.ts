  ...
  test: {
    projects: [
      ...
      { test: { name: 'server', environment: 'node', include: ['tests/server/**/*.test.ts'], env: serverEnv,
                setupFiles: ['tests/setup.ts', 'tests/server/pure-mocks.ts'], testTimeout: 20_000, hookTimeout: 60_000 } },
      ...
      { test: { name: 'db', environment: 'node', include: ['tests/db/**/*.test.ts'], env: serverEnv,
                setupFiles: ['tests/setup.ts'], testTimeout: 20_000, hookTimeout: 60_000 } },
