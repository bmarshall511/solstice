...
- Every `S.tariff?.importRateAllIn ?? .1064` / `?? .0719` becomes `S.rate` / `S.exportRate`. These are set from `S.reconcile.at(-1)?.tariff`, have **no fallback**, and are null for guests (F6). The owner keeps his fallback only through the server, so his view is unchanged.
